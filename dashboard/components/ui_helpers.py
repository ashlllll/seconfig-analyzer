...
# ---------------------------------------------------------------------------
# Internal helper — normalise dict / dataclass access
# ---------------------------------------------------------------------------

def _attr(obj: Any, key: str, default: Any = "") -> Any:
    """
    Read *key* from *obj* whether it is a plain dict or a dataclass/object.

    Using this everywhere removes the need for scattered ``isinstance(obj, dict)``
    checks and makes the UI independent of the exact model representation.
    """
...
def severity_badge(severity: str) -> str:
    ...
    sev = severity.lower()
    colour, bg, border = SEVERITY_COLOURS.get(
        sev, ("#6b8299", "rgba(107,130,153,0.15)", "rgba(107,130,153,0.35)")
    )
    return (
...
def category_badge(category: str) -> str:
    ...
    colour, bg = cat_colours.get(
        category.lower(), ("#6b8299", "rgba(107,130,153,0.1)")
    )
    return (
        f'<span style="display:inline-block;padding:2px 8px;border-radius:4px;'
...
def section_header(title: str, subtitle: str = "", icon: str = "") -> None:
    ...
    icon_html = (
        f'<span style="font-size:1.4rem;margin-right:10px;">{icon}</span>'
        if icon else ""
    )
    sub_html = (
        f'<p style="color:#6b8299;font-size:13px;margin-top:4px;margin-bottom:0;">'
        f'{subtitle}</p>'
        if subtitle else ""
    )
    st.markdown(
...
def metric_card(
    label: str, value: str, delta: str = "", delta_colour: str = "normal"
) -> None:
...
def issue_card(issue: Any, index: int = 0) -> None:
    """
    Render an expandable issue card.

    Accepts either a plain ``dict`` or a ``SecurityIssue`` dataclass — uses
    :func:`_attr` internally so callers do not need to serialise first.
    """
    sev   = _attr(issue, "severity", "info")
    title = _attr(issue, "title", "Unknown Issue")
    icon  = SEVERITY_ICONS.get(sev.lower(), "⚪")
    rule  = _attr(issue, "rule_id", "")
    line  = _attr(issue, "line_number", "?")
    ...
    with st.expander(label, expanded=False):
        ...
        with col2:
            st.markdown(
                f"**Category:** {category_badge(_attr(issue, 'category', ''))}",
                unsafe_allow_html=True,
            )
        with col3:
            nist      = _attr(issue, "nist_function", "")
            cwe       = _attr(issue, "cwe_id", "")
        ...
        st.markdown("---")
        desc = _attr(issue, "description", "")
        if desc:
            st.markdown(f"**Description:** {desc}")

        vuln_code = _attr(issue, "vulnerable_code", "")
        if vuln_code:
            st.markdown(f"**Vulnerable Code** (line {line}):")
            st.code(vuln_code, language="bash")

        hint = _attr(issue, "remediation_hint", "")
        if hint:
...
def fix_card(fix: Any) -> bool:
    """
    Render a fix card with code diff.

    Accepts either a plain ``dict`` or a ``SecurityFix`` dataclass.

    Returns ``True`` if the checkbox inside the card is currently checked.
    The checkbox state is stored in ``st.session_state`` keyed by
    ``fix_<fix_id>``, so it persists across Streamlit reruns regardless of
    whether the expander is open or collapsed.
    """
    issue_title = _attr(fix, "issue_title", "Unknown")
    fix_type    = _attr(fix, "fix_type", "manual")
    priority    = _attr(fix, "priority", "medium")
    fix_id      = _attr(fix, "fix_id", str(id(fix)))
    ...
    # The checkbox widget key — must be stable across reruns.
    chk_key = f"fix_{fix_id}"

    with st.expander(f"{type_icon} {issue_title}", expanded=False):
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(
                f'<span style="color:{colour};font-family:\'JetBrains Mono\','
                f'monospace;font-size:12px;">⬤ {fix_type.replace("_"," ").upper()}</span>',
                unsafe_allow_html=True,
            )
        with col2:
            st.markdown(f"**Priority:** `{priority}`")
        with col3:
            effort = _attr(fix, "effort", "medium")
            st.markdown(f"**Effort:** `{effort}`")

        st.markdown("---")

        original = _attr(fix, "original_code", "")
        fixed    = _attr(fix, "fixed_code", "")
        ...
        side_effects = _attr(fix, "side_effects", [])
        if side_effects:
            st.warning("⚠️ **Side effects:** " + " · ".join(side_effects))

        # Checkbox state lives in session_state — persists whether the
        # expander is open or closed, and is readable after the function
        # returns regardless of the expander's current expansion state.
        st.checkbox("Apply this fix", key=chk_key)

    # Read from session_state AFTER the expander block so the return value
    # is always current, even when the expander is collapsed.
    return bool(st.session_state.get(chk_key, False))
...
def risk_colour(score: float) -> str:
    ...
    if score >= 80:   return "#f04f47"
    elif score >= 60: return "#e88c3a"
    elif score >= 40: return "#d9a83a"
    elif score >= 20: return "#3dba6e"
    else:             return "#3b8ef3"
...
def render_global_footer() -> None:
    ...
    Injects its own CSS so the main content is never hidden behind the bar.
    """
    st.markdown(
        """
        <style>
        ...
        @media (max-width: 768px) {
            ...
            .secconfig-global-footer-inner { font-size:9px; padding:8px 12px 9px; }
        }
        </style>
...
# ---------------------------------------------------------------------------
# Sidebar branding (legacy helper — sidebar.py uses its own renderer)
# ---------------------------------------------------------------------------
